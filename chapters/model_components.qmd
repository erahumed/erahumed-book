...
be drain water into a single ditch - meaning that the sum of 
...
\dfrac{\max \lbrace 0, Q(t)-\sum_{c'}O_{c,t}^\text{id}\rbrace}{N_C},\\
I_{c,t} &= \max\lbrace I_{c,t}^\text{id}-O_{c,t}^\text{id} + O_{c,t},\,0 \rbrace
\end{split}
$$ {#eq-hbp-real-flows}

In words, clusters are emptied in a random order within the allowed capacity of 
the corresponding ditch; if the sum of ideal outflows is less than capacity, the 
remaining outflow volume is shared uniformly among clusters. Using 
@eq-hbp-real-flows, we finally determine the real water level achieved as:
