...
In addition to these rather complex input, the model component requires the 
...
In the notation introduced above, the purpose of this model layer is to simulate 
...
where the lake's inflow is the one computed by the HBA model component (see 
@sec-hba).
...
[^model_components-3], and the real flows are calculated as:

[^model_components-3]: With some abuse of notation, we assume the indexes $c$ and $c'$ in @eq-hbp-real-flows to be sorted according to this random permutation.
...
This model component describes the application of chemicals to rice fields 
based on the simulated hydrology computed by the previous layer.
...
HBP model component (*cf.* Sec TODO), so that $V_i = V(t) + O(t)$, where $O(t)$
